...
#define FTP_ERROR(...)                                                  \
  do                                                                    \
    {                                                                   \
      ...
      errorBuffer = vp_os_calloc (COMMAND_BUFFER_SIZE, 1);        \
      if (NULL != errorBuffer)                                          \
        {                                                               \
...
waitFor226Answer (_ftp_t *ftp)
{
  ...
  char *srvMsg = (char *)vp_os_calloc (MAX_SIZE_MSG, 1);
  if (NULL == srvMsg)
    {
      return FTP_FAIL;
    }
...
{
	_ftp_status ftp_result;
  char *ftpAnswer = (char *)vp_os_calloc (COMMAND_BUFFER_SIZE, 1);
  if (NULL == ftpAnswer)
    {
      return FTP_FAIL;
    }
  ftp_result = ftpTransfert (ftp, "TYPE I\r\n\0", ftpAnswer, COMMAND_BUFFER_SIZE-1);
...
getFileSize (_ftp_t *ftp, const char *distPath)
{
  ...
  int repCode = 0;
  _ftp_status ftp_result;
  char *ftpCommand = (char *)vp_os_calloc (COMMAND_BUFFER_SIZE, 1);
  char *ftpAnswer = (char *)vp_os_calloc (COMMAND_BUFFER_SIZE, 1);
  ...
  ftp_result = ftpTransfert (ftp, ftpCommand, ftpAnswer, COMMAND_BUFFER_SIZE-1);
  if (FTP_FAILED (ftp_result))
    {
      FTP_ERROR ((char *)"Unable to get file size\n");
      vp_os_free (ftpCommand);
      vp_os_free (ftpAnswer);
      return -1;
    }
...
getPassiveIpAndPort (const char *response, char *ip, int *port, int ipLen)
{
  ...
  numread = sscanf (&response[indexOfFirstIpField], "%d,%d,%d,%d,%d,%d)", &ip1, &ip2, &ip3, &ip4, &port1, &port2);
...
ftpTransfert (_ftp_t *ftp, const char *message, char *answer, int answSize)
{
  ...
  flushFtp (ftp);
  ftp_result = ftpSend (ftp, message);
  if (FTP_FAILED (ftp_result))
    {
      return ftp_result;
    }
...
ftpSend (_ftp_t *ftp, const char *message)
{
  ...
  vp_result = ftp->writeSock (ftp->socket, (uint8_t *)message, &bytes);
...
ftpConnect (const char *ip, int port, const char *username, const char *password, _ftp_status *status)
{
  ...
  isAnonymous = ((0 == strcmp (username, "anonymous")) || (0 == strcmp (username, ""))) ? 1 : 0;
  if (1 == isAnonymous)
    {
  ...
  *status = FTP_FAIL;
  retFtp = vp_os_malloc (sizeof (_ftp_t));
  if (NULL == retFtp)
    {
  ...
  vp_result = vp_com_open_socket (retFtp->socket, &(retFtp->readSock), &(retFtp->writeSock));
  if (VP_FAILED (vp_result))
    {
  ...
  result = setSockTimeout ((int)retFtp->socket->priv, SOCK_TO_SEC, SOCK_TO_USEC);
  if (0 > result)
    {
...
DEFINE_THREAD_ROUTINE (ftpList, param)
{
  ...
  ftp_result = ftpTransfert (params->ftp, "PASV\r\n\0", srvMsg, MAX_SIZE_MSG-1);
  if (FTP_FAILED (ftp_result))
    {
      FTP_ERROR ("Error while sending command\n");
  ...
  vp_result = vp_com_open_socket (dataSocket, &dataRead, &dataWrite);
  if (VP_FAILED (vp_result))
    {
      FTP_ERROR ("Unable to connect\n");
      CLEAN_PARAMS (FTP_FAIL);
  ...
  result = setSockTimeout ((int)dataSocket->priv, SOCK_TO_SEC, SOCK_TO_USEC);
  if (0 > result)
    {
  ...
  loc226Status = waitFor226Answer (params->ftp);
  flushFtp (params->ftp);
...
DEFINE_THREAD_ROUTINE (ftpGet, param)
{
  ...
  fileSize = getFileSize (params->ftp, params->remoteName);
  if (0 >= fileSize)
    {
  ...
  sizeToGet = fileSize;
  localFileSize = getLocalFileSize (params->localName);
  appendToFile = params->useResume;
  appendOffset = 0;
  ...
  while (sizeGot < fileSize)
    {
      ...
      vp_result = dataRead (dataSocket, (uint8_t *)filePart, &bytes);
      if (VP_FAILED (vp_result))
        {
...
DEFINE_THREAD_ROUTINE (ftpPut, param)
{
	_ftp_status loc226Status;
	int numSend;
	int numberOfFullSendNeeded;
  int partialSendNeeded;
	float percentSend;
	int sizeSent;
	int result;
	C_RESULT vp_result;
  ...
  sizeSent = appendOffset;
  percentSend = (sizeSent * 100.0) / (localFileSize *1.0);
  params->callback (FTP_PROGRESS, (void *)&percentSend, params->ftp);

  numberOfFullSendNeeded = (sizeToPut / (MAX_SIZE_MSG-1));
  partialSendNeeded = (sizeToPut % (MAX_SIZE_MSG-1)); // Zero if not needed, non-zero if needed
  filePart = vp_os_calloc (MAX_SIZE_MSG, 1);
  if (NULL == filePart)
    {
      FTP_ERROR ("Unable to alloc buffer \n");
  ...
  for (numSend = 0; numSend < numberOfFullSendNeeded; numSend++)
    {
		int bytes;
      CHECK_ABORT;
      bytes = fread (filePart, 1, MAX_SIZE_MSG-1, localFile);
      if (MAX_SIZE_MSG-1 != bytes)
        {
          FTP_ERROR ("Unable to read from file\n");
          CLEAN_PARAMS (FTP_FAIL);
        }
      vp_result = dataWrite (dataSocket, (uint8_t *)filePart, &bytes);
      if (VP_FAILED (vp_result))
        {
          FTP_ERROR ("Unable to send data\n");
          CLEAN_PARAMS (FTP_FAIL);
  ...
  if (0 != partialSendNeeded)
    {
      ...
      sendBytes = bytes;
      vp_result = dataWrite (dataSocket, (uint8_t *)filePart, &sendBytes);
      if (VP_FAILED (vp_result))
        {
          FTP_ERROR ("Unable to send data\n");
          CLEAN_PARAMS (FTP_FAIL);
...
{
	_ftp_status threadReturn;
  THREAD_HANDLE putThread;
  _ftp_put_param *param;
  ...
  param = vp_os_malloc (sizeof (_ftp_put_param));
...
ftpGet (_ftp_t *ftp, const char *remoteName, const char *localName, int useResume, ftp_callback callback)
{
  ...
  param = vp_os_malloc (sizeof (_ftp_get_param));
...
ftpList (_ftp_t *ftp, char **fileList,  ftp_callback callback)
{
  ...
  actualCallback = (NULL != callback) ? callback : emptyCallback;
  if (NULL == ftp)
    {
      actualCallback (FTP_FAIL, NULL, ftp);
      return FTP_FAIL;
  ...
  param = vp_os_malloc (sizeof (_ftp_list_param));
