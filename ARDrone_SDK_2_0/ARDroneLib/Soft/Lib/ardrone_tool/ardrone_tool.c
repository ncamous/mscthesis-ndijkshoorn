...
#include <ardrone_tool/ardrone_version.h>
	ardrone_version_t ardroneVersion = {1,2,3}; // Iran Open hack: assume it is an ARDrone 1 for the moment
...
static uint64_t ArdroneToolRefreshTimeInUs = ARDRONE_REFRESH_MS * 1000;
...
#else
		vp_os_delay_us((uint32_t )(ArdroneToolRefreshTimeInUs - delta));
#endif
	}
...
int ardrone_tool_main(int argc, char **argv)
{
...
#ifdef _WIN32
	  if(drone_ip_address && inet_addr(drone_ip_address)!=INADDR_NONE)
#else
  ...
  while (getDroneVersion (root_dir, wifi_ardrone_ip, &ardroneVersion) != 0)
    {
	...
	else
	{
		...
		*/
		for (lastSlashPos = (int )strlen (argv[0])-1; lastSlashPos > 0 && argv[0][lastSlashPos] != '/' && argv[0][lastSlashPos] != '\\'; lastSlashPos--);
