...
#define _STDINT // to prevent inclusion of stdint.h
#include <impl/framework.h> // has to go first, because it includes winsock2.h, which prevents loading winsock.h
#endif
...
//#include <impl/getifaddrs.h> // only framework is allowed to be included directly
...
#ifndef getifaddrs
#define getifaddrs(adress) (pgm_getifaddrs((struct pgm_ifaddrs_t **)(adress), NULL))
#endif

#ifndef freeifaddrs
#define freeifaddrs(adress) (pgm_freeifaddrs((struct pgm_ifaddrs_t *)(adress)))
#endif
...
C_RESULT wifi_server_auth(struct in_addr *addr)
{
	...
	// from 1.8 adrone_navdata_client.c
#ifdef _WIN32
		if (setsockopt((int32_t)socket.priv, SOL_SOCKET, SO_RCVTIMEO, (const char*)&timeout_for_windows, sizeof(timeout_for_windows)) < 0)
#else
    ...
    do
    {
    	...
    	{
			int length = (int )len;
    		if (VP_FAILED(write(&socket, msg, &length)))
			{
				vp_com_close(COM_AUTH(), &socket);
				vp_com_shutdown( COM_AUTH() );
				return C_FAIL;
...
vp_com_config_t* wifi_config(void)
{
	...
	for (; (char *) ifr < (char *) ifc.ifc_req + ifc.ifc_len; ++ifr) {
		...
		{
			inet_ntop(AF_INET, &addr, config.localHost, VP_COM_NAME_MAXSIZE-1);
			...
			if (0 == ioctl(sockfd, SIOCGIFNETMASK, ifr) && strcmp(
				...
				addr = inaddrr(ifr_addr.sa_data);
				inet_ntop(AF_INET, &addr, config.netmask, VP_COM_NAME_MAXSIZE-1);
			}
			...
			if (ifr->ifr_flags & IFF_BROADCAST) {
				if (0 == ioctl(sockfd, SIOCGIFBRDADDR, ifr) && strcmp("0.0.0.0",
					...
					addr = inaddrr(ifr_addr.sa_data);
					inet_ntop(AF_INET, &addr, config.broadcast, VP_COM_NAME_MAXSIZE-1);
				}
			}
...
vp_com_config_t* wifi_config(void)
{
	...
	if(strlen(config.itfName) == 0)
	{
		...
		while (!found && (ifAddrStruct != NULL))
		{
			...
			if (ifAddrStruct->ifa_addr->sa_family == AF_INET && strncmp(ifAddrStruct->ifa_name,"{",1) != 0)
			{
				tmpAddr = ((struct sockaddr_in *)ifAddrStruct->ifa_addr)->sin_addr;
				if ( (ntohl(tmpAddr.s_addr) & 0xFFFFFF00) == WIFI_BASE_ADDR )
				{
					inet_ntop(AF_INET, &tmpAddr, config.localHost, VP_COM_NAME_MAXSIZE-1);
					...
					inet_ntop(AF_INET, &tmpAddr, config.netmask, VP_COM_NAME_MAXSIZE-1);
					tmpAddr = ((struct sockaddr_in *)ifAddrStruct->ifa_broadaddr)->sin_addr;
					inet_ntop(AF_INET, &tmpAddr, config.broadcast, VP_COM_NAME_MAXSIZE-1);
					found = TRUE;
				}
			}
...
{
	//printf("wifi_config_socket: size of vp_com_socket_t %d\n", sizeof(vp_com_socket_t));
