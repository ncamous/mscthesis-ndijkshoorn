...
bool_t ardrone_academy_navdata_check_usb_record_status (void)
{
	bool_t retVal;
    LOCK_ND_MUTEX ();
	retVal = navdata_state.droneStoppedUsbRecording;
...
bool_t ardrone_academy_navdata_check_takeoff_cancelled (void)
{
	bool_t retVal;
    LOCK_ND_MUTEX ();
    retVal = navdata_state.takeOffWasCancelled;
...
C_RESULT ardrone_academy_navdata_process( const navdata_unpacked_t* const pnd )
{
    ...
    if (C_OK == TRYLOCK_ND_MUTEX ())
    {
		...
		bool_t currentDroneUsbRecording;
        ...
        currentDroneUsbRecording = (NAVDATA_HDVIDEO_USBKEY_IS_RECORDING == (NAVDATA_HDVIDEO_USBKEY_IS_RECORDING & pnd->navdata_hdvideo_stream.hdvideo_state));
