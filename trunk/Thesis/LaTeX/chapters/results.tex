...
The next experiment checks the acceleration of the real and simulated AR.Drone. First i give an estimate of how quickly the AR.Drone's controller changes its pitch to match the commanded pitch and how well it can keep it.
For this i select all samples from 100ms after the start
of the 2s, 3s and 5s pulses till the commanded pitch has been reached.
This
...
%For each of these spans i estimate the tangent:
...
\caption{Photo of the sports floor, which resambles the circumstances encountered during the IMAV competitions.}
\label{fig:exp1-floor}
\end{figure}
...
\begin{table}[htb!]
	...
	Method & Mean absolute error (\small{mm}) & Mean relative error (percentage of trajectory length) \\
        \hline
        onboard velocity (AV) & 988 & 0.960\% \\
	onboard position (AP) & 988 & 0.960\% \\
	visual odometry (VO) & 4251 & 4.172\% \\
	AV + localization & 461 & 0.452\% \\
	VO + localization & 1158 & 1.136\% \\
	\hline
    \end{tabular}
    \caption{Errors made by the position estimation methods when flying in circumstances encountered during the IMAV competitions.}
...
\caption{Estimated trajectories of the different position estimation methods. The AR.Drone flew three 8-shapes above a gym floor. All units are in \small{mm}.}
...
The AR.Drone's onboard velocity (AV) is able to estimate the velocitiy quite well, with an average error of $1\small{m}$.
...
These results confirm that the localization method is able to deal with circumstances encountered during the IMAV competitions.
A feature-based approach to visual odometry is insufficient to estimate the velocity above all areas of a gym floor.
