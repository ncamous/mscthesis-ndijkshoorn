...
The EKF state vector comprises a position vector $p^{W}$, velocity vector $v^{W}$, acceleration vector $a^{W}$, attitude (orientation) vector $q^{W}$ and altitude vector $h^W$ (Section \ref{sec:pose_estimation}).
...
However, the ultrasound's altitude measurement can be integrated into the EKF to improve the altitude estimate.
The ultrasound sensor is not sensitive to drift because it provides an absolute altitude measurement.
Relying only on the ultrasound sensor is not optimal since the measurements depend heavily on material and structure of the floor.
