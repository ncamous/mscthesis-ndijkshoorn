...
The corresponding node is augmented to the graph accordingly.
...
However, this article and Steder's other publications don't describe how the visual map is constructed and visualized.
...
In cases where a priori maps are not available, a mechanism must exist for building a map while simultaneously using that map for the purposes of localization.
...
The distance measurements received by the ultrasound are then processed to identify distinctive shapes which are used to initialize new features in the SLAM map.
...
Their method classifies the type of indoor environment (e.g., corridors, staircases, rooms and corners) and then uses vision algorithms based on perspective cues to estimate the desired direction to fly.
...
Staircases can be navigated by finding the center of the staircase, so the AR.Drone can follow the staircase using its onboard intelligence (i.e., altitude stabilization).
